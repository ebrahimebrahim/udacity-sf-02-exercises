...
void projectLidarToCamera2()
{
    ...
    // projective camera coords to projective image coords:
    cv::Mat P_rect_00(3,4,cv::DataType<double>::type); // 3x4 projection matrix after rectification
    // projective stereo image coords to projective mono image coords:
    cv::Mat R_rect_00(4,4,cv::DataType<double>::type);
    // projective lidar coords to projective camera coords:
    ...
    // overall lidar to image projection
    cv::Mat lidar_to_img(3,4,cv::DataType<double>::type);
    lidar_to_img = P_rect_00 * R_rect_00 * RT;

    // TODO
    for(auto it=lidarPoints.begin(); it!=lidarPoints.end(); ++it) {

        // skip points that are too low (road) or behind the car or too far out in front or too far off to the side or too low intensity:
        ...
        cv::Point pt;
        pt.x = Y.at<double>(0,0) / Y.at<double>(2);
        pt.y = Y.at<double>(1,0) / Y.at<double>(2);
